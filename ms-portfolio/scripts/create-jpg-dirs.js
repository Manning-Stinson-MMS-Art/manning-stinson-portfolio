...
async function addJpgDirectories(bucketName) {
  try {
    ...
    for (const prefix of CommonPrefixes) {
      ...
      const jpgExists = await verifyDirectory(bucketName, `${dirPath}jpg/`);
      
      if (jpgExists) {
        logger.info(`jpg directory already exists in ${baseName}/`);
        results.skipped++;
        continue;
      }

      try {
        const jpgDirKey = `${dirPath}jpg/.keep`;
        ...
        const wasCreated = await verifyDirectory(bucketName, `${dirPath}jpg/`);
        if (wasCreated) {
          logger.info(`Created jpg directory in ${baseName}/`);
          results.created++;
        } else {
          logger.error(`Failed to verify creation of jpg directory in ${baseName}/`);
          results.errors++;
        }

      } catch (error) {
        logger.error(`Error creating jpg directory in ${baseName}: ${error}`);
        results.errors++;
      }
...
async function main() {
  try {
    ...
    await addJpgDirectories(bucketName);
  } catch (error) {
    logger.error('Fatal error:', error);
    process.exit(1);
  }
